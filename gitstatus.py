...
nb_untracked = len([0 for status in Popen(['git','status','--porcelain','--untracked-files=all'],stdout=PIPE).communicate()[0].decode("utf-8").splitlines() if status.startswith('??')])
